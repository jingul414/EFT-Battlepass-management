...
import {
  ...
  FileText, LockKeyhole, MapPinned, Minus, Plus, RotateCcw, ShieldCheck,
  Target, Trash2,
} from 'lucide-react';
...
import {
  DOCUMENTS, MODE_LABELS, MODE_LIMITS, REWARDS, TOTAL_COST,
          ...
          <section className="panel p-4">
            ...
            <div className="grid grid-cols-2 gap-2 sm:grid-cols-4 lg:grid-cols-1">
              {DOCUMENTS.map((document) => (
                <div key={document.id} className="inventory-row">
                  <div className="flex min-w-0 items-center gap-2"><span className="size-1.5 shrink-0 rounded-full" style={{ backgroundColor: document.color }} /><span className="truncate text-xs text-[#c7c9c1] sm:text-sm">{document.shortLabel}</span></div>
                  <div className="flex items-center gap-1">
                    <Button aria-label={`${document.label} 1개 감소`} className="inventory-step" size="icon-xs" variant="ghost" onClick={() => updateInventory(document.id, state.inventory[document.id] - 1)}><Minus /></Button>
                    <Input aria-label={`${document.label} 수량`} className="h-7 w-11 rounded-sm border-white/8 px-1 text-center font-mono text-xs" inputMode="numeric" min={0} type="number" value={state.inventory[document.id]} onChange={(event) => updateInventory(document.id, Number(event.target.value))} />
                    <Button aria-label={`${document.label} 1개 증가`} className="inventory-step" size="icon-xs" variant="ghost" onClick={() => updateInventory(document.id, state.inventory[document.id] + 1)}><Plus /></Button>
                  </div>
                </div>
              ))}
            </div>
            <div className="mt-3 flex items-center justify-between rounded-sm border border-[#c9a96a]/22 bg-[#c9a96a]/7 px-3 py-2.5">
              <span className="text-sm text-[#d7c291]">기밀 문서</span>
              <Input aria-label="기밀 문서 수량" className="h-7 w-14 rounded-sm border-[#c9a96a]/20 text-center font-mono text-[#ecd296]" min={0} type="number" value={state.classified} onChange={(event) => setState((current) => ({ ...current, classified: clampCount(Number(event.target.value)) }))} />
            </div>
        ...
        <section className="min-w-0 space-y-5">
          ...
          <section className="panel overflow-hidden">
            ...
            <div className="divide-y divide-white/7">
              {selectedRewards.map((reward) => <RewardRow key={reward.id} reward={reward} state={state} status={rewardStatus(reward, state, selectedPageUnlocked)} onRequirementsChange={setRequirements} onToggleClaim={toggleClaim} />)}
            </div>
          </section>
...
function RewardRow({ reward, state, status, onRequirementsChange, onToggleClaim }: {
  reward: Reward;
  state: TrackerState;
  status: RewardStatus;
        ...
        {requirements.map((requirement, index) => {
          ...
          return (
            <div key={requirement.documentId} className="requirement-row">
              ...
              <Input
                aria-label={`레벨 ${reward.id} ${document.label} 요구 수량`}
              ...
              />
              <Button
                aria-label={`레벨 ${reward.id} ${document.label} 요구 항목 삭제`}
      ...
      <div className="text-left sm:text-right">
        ...
        {status === 'short' && (
          <p className="mt-1 text-[10px] text-[#c98c6c]">
            {Object.entries(deficits).map(([id, count]) => `${DOCUMENTS.find((item) => item.id === id)?.shortLabel} ${count}`).join(' · ')} 부족
          </p>
        )}
      </div>
